...
exports.handler = (event, context, callback) => {
	//async.auto - Determines the best order for running the AsyncFunctions in tasks
    async.auto({
      download: function(callback) {
          ...
          let csvFile = "/tmp/"+key;
          let file = fs.createWriteStream(csvFile); //output to /tmp/key
          s3.getObject(downloadParams).createReadStream().on('error', function(err){//read file in S3
          ...
          file.on('finish', function() {
            ...
            callback(null, {'csvFile':csvFile, 'bucketName':bucket, 'key':key}); //pass to next task
          });
      ...
      csvtojson: ['download', function(results, callback){ //run after download task
          console.log("Inside csvtojson function");
          let csvFile = results.download.csvFile; //from previous task
          csv()
            .fromFile(csvFile)
            .on("end_parsed",function(jsonArrayObj){ //when parse finished, result will be emitted here.
               ...
               jsonfile.writeFile(jsonFile, jsonArrayObj, function (err) { //output the converted json data to json file
                if(err){
                  console.error(err);
                  callback(err);
                }
      ...
      sendToDynamo: ['download', 'csvtojson', function(results, callback) { //run after download, csvtojson tasks
          console.log("Inside sendToDynamo function");
          console.log("Importing data into DynamoDB. Please wait.");
          fs.readFile(results.csvtojson.jsonFile, function (err, data) { //get from csvtojson task
            if (err){
              console.log(err);
              return callback(err);
            }
