    ...
    if (describeTagsErr){
      ...
      ec2.describeVolumes(describeVolParams, function(describeVolErr, describeVolData) {
        if (describeVolErr){
          ...
          async.parallel({
            one: function(oneCB) {
            	//loop each of instanceIDs and call function with (value, key, callback)
                ...
                }, function (imageErr) { //if any item causes error, it will pass error to the final function here
                    if (imageErr){
          ...
          }, function(finalErr, finalResults) { //final function calls after async.parallel
              if(finalErr){
                callback(finalErr);
              }
              callback(null, "Done!!");
