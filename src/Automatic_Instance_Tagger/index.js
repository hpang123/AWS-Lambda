...
function createTag(tagCB){
  ...
  }, function (err) { //final handle the error.
      if (err){
        console.log(err);
        tagCB(err);
      }
...
function runInstances(runCB){
  ...
  async.series({
    one: function(oneCB) { //get ids
      async.forEachOf(items, function (item, key, cb) {
        id.push(item.instanceId);
        cb(null, "added");
      }, function (err) { //error handler for async
          if (err){
            console.log(err);
            oneCB(err); //if error, stop the next step in series.
          }
          else{
    ...
    two: function(twoCB){//make describeParam
      describeParams = {
        InstanceIds: [ 
        ]
      };
    ...
    three: function(threeCB){
      ec2.describeInstances(describeParams, function(err, data) {
        if (err){
          ...
          async.forEachOf(reservations, function (reservation, key, resrvCB) {//process reservation from returned by describeInstances
